...
**场景**: 张明选择执行方式：手动触发或等待Agent自动执行

| 维度 | 内容 |
|------|------|
| **用户行为** | 方式A：点击"⚡ 执行"按钮手动触发<br>方式B：等待Agent后台服务自动执行 |
| **用户目标** | 让AI按照验收标准完成任务 |
| **触点** | Kanban页面 → doing列 → 执行按钮 / Agent后台服务 |
| **用户想法** | "让AI帮我做这个任务，我去做其他事" |
| **情感曲线** | 🤖 期待 → ⚙️ 等待 → 👀 查看进度 |
| **痛点** | ⚠️ 不知道AI在做什么<br>⚠️ 执行时间不确定 |
| **成功标准** | 任务状态显示"执行中"，可查看日志 |
| **对应用户故事** | US-4: 执行任务, US-6: 自动执行任务 |
...
| 维度 | 内容 |
|------|------|
| **用户行为** | 点击"📋"按钮 → 查看执行日志弹窗 → 查看状态、日志、结果 |
| **用户目标** | 了解AI的执行过程和当前进度 |
| **触点** | 日志按钮 → 执行日志弹窗 |
| **用户想法** | "AI做到哪一步了？有没有遇到问题？" |
| **情感曲线** | 🔍 好奇 → 📊 了解 → 😌 放心 |
| **痛点** | ⚠️ 日志太技术化难理解<br>⚠️ 不知道还要等多久 |
| **成功标准** | 能看到清晰的执行日志和状态 |
| **对应用户故事** | US-5: 查看执行日志 |
...
**场景**: 张明确认AI执行结果，移动到done

| 维度 | 内容 |
|------|------|
| **用户行为** | 查看执行结果 → 确认符合验收标准 → 移动任务到done |
| **用户目标** | 标记任务完成，获得成就感 |
| **触点** | 执行日志弹窗 / Kanban页面 → doing列 → 移动到done |
| **用户想法** | "AI帮我搞定了！又完成一个任务！" |
| **情感曲线** | ✅ 满足 → 🎉 成就感（AI帮我完成了！） |
| **痛点** | ⚠️ 需要手动验证AI的结果 |
| **成功标准** | 任务移到done，看板更新，显示庆祝动画 |
| **对应用户故事** | US-7: 查看看板（移动功能） |
...
**场景**: 张明查看done列表，回顾AI帮他完成的任务

| 维度 | 内容 |
|------|------|
| **用户行为** | 查看done列 → 查看执行日志 → 统计完成数量 → 反思改进点 |
| **用户目标** | 了解工作进展，评估AI执行质量 |
| **触点** | Kanban页面 → done列 → 执行日志 |
| **用户想法** | "这周AI帮我完成了多少任务？质量如何？" |
| **情感曲线** | 📊 回顾 → 💡 洞察 → 😊 满意 |
| **痛点** | ⚠️ 缺少统计数据<br>⚠️ 无法评估AI执行质量 |
| **成功标准** | 能看到所有已完成任务和执行记录 |
| **对应用户故事** | US-7: 查看看板, US-5: 查看执行日志 |

**验证检查点**:
- [ ] done列显示所有已完成任务
- [ ] 可以查看任务详情回顾
- [ ] 可以查看任务的执行日志
- [ ] （未来）统计功能：完成数量、平均时间、AI执行成功率等

---

### 阶段 7: 自动化工作流 🤖

**场景**: 张明启动Agent后台服务，实现无人值守
...
| **用户目标** | 实现全自动任务执行，解放双手 |
| **触点** | 命令行 / 后台服务 |
| **用户想法** | "让AI自动处理，我去做更重要的事" |
| **情感曲线** | 🚀 兴奋 → 😌 放心 → 🎯 专注其他工作 |
| **痛点** | ⚠️ 不知道Agent是否正常运行<br>⚠️ 需要监控Agent状态 |
| **成功标准** | Agent持续运行，自动执行任务 |
| **对应用户故事** | US-6: 自动执行任务 |
