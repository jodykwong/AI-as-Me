...
class AgentExecutor:
    ...
    def execute_task(self, task, agent_name: str = None) -> AgentResult:
        ...
        exec_logger = ExecutionLogger(task_id)

        # 更新任务状态为准备中
        self._update_task_phase(task, "PREPARING", 10)
        exec_logger.log("step", "开始任务准备阶段...")
        ...
        # 如果任务有工具配置，优先使用
        if (
            hasattr(task, "clarification")
            and task.clarification
            and task.clarification.tool
        ):
        ...
        exec_logger.log("step", f"分析任务需求，选择Agent: {agent_name or '自动选择'}")
        ...
        if agent_name:
            ...
            if not agent:
                ...
                exec_logger.log("error", error_msg)
        ...
        exec_logger.log("step", f"开始执行，使用Agent: {agent.name}{model_info}")

        # 更新任务状态为执行中
        self._update_task_phase(task, "EXECUTING", 50)
        exec_logger.log("step", "正在执行任务...")

        result = agent.execute(task, model=model)

        # 记录执行结果
        if result.success:
            exec_logger.log("output", f"执行成功: {result.output[:200]}...")
        else:
            exec_logger.log("error", f"执行失败: {result.error}")

        # 更新任务状态为验证中
        self._update_task_phase(task, "VALIDATING", 90)
        exec_logger.log("step", "验证执行结果...")
        ...
        if result.success:
            ...
            exec_logger.log("step", "任务执行完成")
        else:
            self._update_task_phase(task, "FAILED", 0)
            exec_logger.log("step", "任务执行失败")

        self.logger.info(
            f"任务 {task_id} {'成功' if result.success else '失败'} (耗时: {result.duration:.2f}s)"
        )
        return result
    ...
    def execute_with_fallback(self, task, agents: List[str] = None) -> AgentResult:
        ...
        for agent_name in agents:
            ...
            attempts.append(
                {"agent": agent_name, "success": result.success, "error": result.error}
            )
    ...
    def _update_task_phase(self, task, phase: str, progress: int = None):
        ...
        try:
            ...
            try:
                ...
                manager.update_task_phase(task.id, phase, progress)
