...
class Database:
    ...
    def create_task(
        self, task_id: str, description: str, tool: str = None, priority: str = "P2"
    ):
        """创建任务"""
        with self._pool.get_connection() as conn:
            conn.execute(
...
class KanbanDB:
    ...
    # Task operations
    def create_task(
        self, title: str, description: str = None, needs_approval: bool = False
    ) -> Task:
    ...
    def create_execution(
        self, task_id: str, executor: str = "deepseek", prompt: str = None
    ) -> Execution:
    ...
    # Log operations
    def add_log(
        self, execution_id: str, level: str, module: str, event: str, data: Dict = None
    ):
        conn = self._conn()
        conn.execute(
            """
    ...
    def update_task_phase(self, task_id: str, phase: str, progress: int = None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            if progress is not None:
                ...
                conn.execute(
                    "UPDATE tasks SET current_phase = ? WHERE id = ?", (phase, task_id)
                )
            conn.commit()
