...
class VibeKanbanManager:
    ...
    def get_board(self) -> dict:
        ...
        # 为doing任务添加执行结果状态和阶段信息
        for task in board["doing"][:]:  # 使用切片复制，避免迭代时修改列表
            result_file = self.kanban_dir / "doing" / f"{task.id}-result.md"
            status_file = self.kanban_dir / "doing" / f"{task.id}-status.json"
            ...
            if result_file.exists():
                ...
                # 自动移动已完成的任务到 done
                try:
                    self.move_task(task.id, "done")
                    board["doing"].remove(task)
                    board["done"].insert(0, task)  # 插入到 done 列表开头
                except Exception as e:
                    # 移动失败时记录但不中断
                    print(f"Warning: Failed to auto-move completed task {task.id}: {e}")
            else:
                ...
                if status_file.exists():
                    ...
                    try:
                        ...
                        task.current_phase = status_data.get("phase", "EXECUTING")
    ...
    def update_task_phase(self, task_id: str, phase: str, progress: int = None):
        ...
        status_file = self.kanban_dir / "doing" / f"{task_id}-status.json"
        ...
        if phase == "COMPLETED":
            try:
                ...
                print(f"任务 {task_id} 已自动移动到 done 板块")
            except Exception as e:
                print(f"自动移动任务 {task_id} 到 done 失败: {e}")

    def execute_task(
        self, task_id: str, agent_name: str = None, trigger_evolution: bool = True
    ) -> dict:
        """执行任务并保存结果
...
"""
        ...
        if trigger_evolution and result.success:
            ...
            evolution_result = engine.evolve(
                task, result.output, result.success, result.duration
            )
